...
loop.run_until_complete(start_hyper())       
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 5:
            ...
            try:
                ...
                await event.edit("𝚂𝚄𝙲𝙲𝙴𝚂𝚂𝙵𝚄𝙻𝙻𝚈 𝙲𝙷𝙰𝙽𝙶𝙴𝙳 𝙱𝙸𝙾 𝙱𝚈 𝙷𝚈𝙿𝙴𝚁 𝚂𝙿𝙰𝙼 𝙱𝙾𝚃")
            except Exception as e:
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 6:
            ...
            try:
                ...
                await event.edit("𝙹𝙾𝙸𝙽 𝙷𝙾𝙶𝚈𝙰 𝙼𝙺𝙲")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        hyper = ("".leave(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(e.text) == 7:
            bc = hyper[0]
            bc = int(bc)
            text = "𝙻𝙴𝙰𝚅𝙸𝙽𝙶 𝚃𝙷𝙸𝚂 𝚂𝙷𝙸𝚃𝚃𝚈 𝙶𝚁𝙾𝚄𝙿"
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        hyperop = hyper[1:]
        if len(hyperop) == 2:
            message = str(hyperop[1])
            counter = int(hyperop[0])
...
async def help(e):
    if e.sender_id in SMEX_USERS:
       text = "𝐀𝐕𝐀𝐈𝐋𝐀𝐁𝐋𝐄 𝐂𝐎𝐌𝐌𝐌𝐀𝐍𝐃𝐒 𝐈𝐍 𝙷𝚈𝙿𝙴𝚁 𝐒𝐏𝐀𝐌 𝐁𝐎𝐓 \n\n𝐔𝐓𝐈𝐋𝐒 𝐂𝐎𝐌𝐌𝐀𝐍𝐃:\n.alive\n.ping\n.restart\n\n𝐔𝐒𝐄𝐑𝐁𝐎𝐓 𝐂𝐎𝐌𝐌𝐀𝐍𝐃:\n.bio\n.join\n.leave\n.pjoin\n.pleave\n\n𝐒𝐏𝐀𝐌 𝐂𝐎𝐌𝐌𝐀𝐍𝐃:\n.spam\n.delayspam\n.bigspam\n.raid\n.replyraid\n.dreplyraid\n\n\nFor more help regarding usage of plugins type plugins name"
       await e.reply(text, parse_mode=None, link_preview=None )
...
print(text)
print("")
print("SMEX! 𝙷𝚈𝙿𝙴𝚁 Multi Spam Bot Started Sucessfully.")
